...
{
	ssd1306_i2c_init(dev, width, height);
...
void ssd1306_show_buffer(SSD1306_t * dev)
{
	for (int page=0; page<dev->_pages;page++) {
		ssd1306_i2c_display_image(dev, page, 0, dev->_page[page]._segs, dev->_width);
	}
}
...
{
	ssd1306_i2c_display_image(dev, page, seg, images, width);
...
ssd1306_display_text_x3(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	...
	for (int nn = 0; nn < _text_len; nn++) {
		...
		for (int yy = 0; yy < 3; yy++)	{ // for each group of 8 pixels high (y-direction)
			...
			ssd1306_i2c_display_image(dev, page+yy, seg, image, 24);
...
{
	ssd1306_i2c_contrast(dev, contrast);
}
...
{
	ssd1306_i2c_hardware_scroll(dev, scroll);
}
...
void ssd1306_wrap_arround(SSD1306_t * dev, ssd1306_scroll_type_t scroll, int start, int end, int8_t delay)
{
	...
	if (delay >= 0) {
		for (int page=0;page<dev->_pages;page++) {
			ssd1306_i2c_display_image(dev, page, 0, dev->_page[page]._segs, 128);
